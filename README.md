...
For storage that has an in-game contents screen — your **stash**, the gunsmith's **gun-parts storage**, and the **Cases** — you don't have to type it all in. Hit **Scan from screen** and a capture frame appears in the headset: aim it at the contents screen and **pull the controller trigger** once per scroll position. Each shot is read and the overlapping ones are merged into a single list, so you just trigger-scroll-trigger your way down the screen — no keyboard needed (though **SPACE** still grabs a shot too, handy at the desk). A centered window shows the running tally live, and **Finish & review** opens a before/after diff (new, changed, and removed items) — alongside the rows it actually read, so you can drop any that look wrong — before you commit. Applying a scan **replaces** that container's contents with what was read — the review step spells that out up front. (The gunsmith storage shows *short* part names that differ from the catalog's full names — the scanner bridges that automatically, so `AR-308 DMR` on the grid still lands on the right catalog part.)
