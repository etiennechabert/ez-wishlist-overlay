...
- **Screenshot OCR** — captures the SteamVR compositor mirror texture and runs one of two pipelines. The **upgrade-panel** path (`ocr::pipeline`) identifies the Facility Upgrade panel and reads owned counts via per-digit template matching, writing them back to the wishlist with an in-headset feedback overlay. The **box-scan** path (`ocr::box_scan`) reads a `Case`'s contents screen as a tile grid and stitches a series of overlapping scroll captures into one item list (REPLACE-on-apply, behind a diff-preview). The stitch/clustering core is platform-independent and unit-tested on every target; the OCR geometry is Windows-gated.
