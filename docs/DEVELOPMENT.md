# Development

Build-from-source, data-refresh, and release notes for EZ Wishlist Overlay. For what the app does and how to install it, see the [README](../README.md).

<p>
  <img src="https://img.shields.io/badge/Rust-stable-DEA584.svg?logo=rust&logoColor=white" alt="Rust">
  <a href="https://sonarcloud.io/summary/new_code?id=etiennechabert_ez-wishlist-overlay"><img src="https://sonarcloud.io/api/project_badges/measure?project=etiennechabert_ez-wishlist-overlay&metric=security_rating" alt="Security Rating"></a>
  <a href="https://sonarcloud.io/summary/new_code?id=etiennechabert_ez-wishlist-overlay"><img src="https://sonarcloud.io/api/project_badges/measure?project=etiennechabert_ez-wishlist-overlay&metric=reliability_rating" alt="Reliability Rating"></a>
  <a href="https://sonarcloud.io/summary/new_code?id=etiennechabert_ez-wishlist-overlay"><img src="https://sonarcloud.io/api/project_badges/measure?project=etiennechabert_ez-wishlist-overlay&metric=sqale_rating" alt="Maintainability Rating"></a>
</p>

---

## Status

Shipping as of v0.4.0:

- **Dataset** — `data.json` + normalized PNG icons committed into the app crate, hand-maintained and validated against the in-game captures under `screenshots/`. (Originally bootstrapped from the upstream [ExfilZone Assistant](https://github.com/zelengeo/exfil-zone-assistant) repo by a scraper crate, retired in #162 — upstream lagged seasons behind the game and carried wrong/duplicate item names.)
- **Desktop GUI** — egui app with a Hideout tab, a Containers tab, and an Items DB catalog, the aggregated wishlist preview pane, recipe overrides + "Export corrections", Settings, and an in-app update check.
- **Containers** — the stash plus user-defined secondary containers (`ContainerKind` = `Case` / `Shelf` / `Bag`), each holding its own contents that sum into owned totals alongside the stash. `Case` containers are scannable; the others are entered by hand.
- **VR overlay** — OpenVR session with pitch-driven show/hide, world-space anchoring, controller laser-pointer clicks + haptics, and live re-render on state change.
- **Screenshot OCR** — captures the SteamVR compositor mirror texture and runs one of two pipelines. The **upgrade-panel** path (`ocr::pipeline`) identifies the Facility Upgrade panel and reads owned counts via per-digit template matching, writing them back to the wishlist with an in-headset feedback overlay. The **box-scan** path (`ocr::box_scan`) reads a `Case`'s contents screen as a tile grid and merges a series of overlapping scroll captures into one item list by **row uniqueness** — each grid row is deduped by the items composing it (tolerant of one drifted/missing tile), so scroll distance and clipped boundary rows no longer break a scan (REPLACE-on-apply, behind a diff-preview where you can drop a mis-read row). The merge/clustering core is platform-independent and unit-tested on every target; the OCR geometry is Windows-gated.
- **Distribution** — MSI installer + portable exe ship via [`release.yml`](../.github/workflows/release.yml) on tag push; PR builds validate the MSI to keep it green. Code signing + an auto-update channel are still pending (the SmartScreen prompt documented in the README is a consequence of the missing signature).

The original engineering spec is in [SPEC.md](../SPEC.md) and deferred decisions in [OPEN_QUESTIONS.md](../OPEN_QUESTIONS.md). Note that the spec predates the OCR work — it listed screenshot capture as a v1 non-goal — so treat it as historical context where it disagrees with what's shipping.

---

## Repo layout

```
crates/
  app/                     # the shipped binary (egui desktop + SteamVR overlay)
crates/app/src/assets/     # embedded data.json + icons (committed, hand-maintained)
docs/                      # README screenshots + this file
LICENSES/                  # third-party license texts we ship
SPEC.md
OPEN_QUESTIONS.md
```

---

## Prerequisites

The repo builds against `stable-x86_64-pc-windows-gnullvm` with the [LLVM-MinGW](https://github.com/mstorsjo/llvm-mingw) toolchain on PATH:

```powershell
winget install MartinStorsjo.LLVM-MinGW.UCRT
rustup default stable-x86_64-pc-windows-gnullvm
```

(MSVC also works once Visual Studio Build Tools with the C++ workload is installed. There's no toolchain pin — pick whichever you prefer.)

The VR layer depends on the [`openvr`](https://crates.io/crates/openvr) crate, whose `openvr_sys` build script compiles Valve's OpenVR C++ SDK via cmake + bindgen. On Windows that also requires:

```powershell
winget install Kitware.CMake
winget install LLVM.LLVM       # for libclang on PATH (bindgen)
```

The VR layer is gated behind `cfg(target_os = "windows")` — macOS and Linux builds skip it entirely and the app falls back to "VR: unavailable on this OS", so the desktop GUI stays iterable cross-platform.

> **Heads up:** VR-enabled builds must use the **MSVC** target. The default gnullvm toolchain can't build `openvr_sys` — it rejects MSVC's `/DWIN32` cxxflag — so the cargo aliases below pin `x86_64-pc-windows-msvc`. gnullvm is only useful for non-VR iteration.

---

## Building & running

From a **VS Developer PowerShell**:

```powershell
cargo app           # release build
cargo app-dev       # debug build, faster compiles for UI iteration
```

From a **plain PowerShell** (uses the bundled launcher to enter the VS Dev Shell + set `LIBCLANG_PATH`):

```powershell
./scripts/run-app.ps1            # debug build
./scripts/run-app.ps1 -Release   # release build
```

See [`scripts/run-app.ps1`](../scripts/run-app.ps1) for the underlying setup.

The window opens at 1200×800. User state persists to `%APPDATA%\etienneb\ez-wishlist-overlay\` on Windows (`state.json`, `overrides.json`, `settings.json`), or the platform equivalent (`~/Library/Application Support/...` on macOS) elsewhere. A sibling `debug/` directory holds this session's throwaway debug artifacts (VR capture screenshots + OCR sidecars); it's flushed empty on every launch and is safe to delete.

### Cargo aliases

The workspace ships aliases in [`.cargo/config.toml`](../.cargo/config.toml) so common commands work the same on Windows, macOS, and Linux without extra tooling:

| Alias              | Expands to                                      |
| ------------------ | ----------------------------------------------- |
| `cargo app`        | `run -p ez-wishlist-overlay --release --target x86_64-pc-windows-msvc` |
| `cargo app-dev`    | `run -p ez-wishlist-overlay --target x86_64-pc-windows-msvc`           |
| `cargo t`          | `test --workspace`                              |
| `cargo c`          | `check --workspace --all-targets`               |
| `cargo l`          | `clippy --workspace --all-targets -- -D warnings` |
| `cargo fmt-check`  | `fmt --all -- --check`                          |

---

## Refreshing game data

`crates/app/src/assets/data.json` + `icons/` are the canonical dataset — edit
them directly. Names, ids and recipes are verified against in-game captures
(`screenshots/`, see `screenshots/CLAUDE.md`); recipe edits must keep the
matching `*.label.txt` fixtures in sync, and the referential-integrity test
(`crates/app/src/data.rs`) rejects recipes pointing at unknown item ids. New
items get a 128×128 PNG under `icons/<item_id>.png`.

After editing, rebuild the app — `data.json` and `icons/` are embedded via
`rust-embed` at compile time.

(History: the dataset was bootstrapped by a `crates/scraper` crate that pulled
the upstream ExfilZone Assistant repo; it was retired in #162 — see
`crates/app/src/assets/SOURCE.md` for the bootstrap provenance.)

---

## Releasing

The version lives in exactly one place — `crates/app/Cargo.toml` — and every
PR bumps it ahead of the last release (CI's `version-bump` gate). A release tag
must equal that version. **Don't hand-type the tag** (`git tag v0.1.0`): typing
"last tag + 1" drifts behind `Cargo.toml` and tags the wrong number — that's
what produced the dangling `v0.4.2` and `v0.4.4` tags that failed
[`release.yml`](../.github/workflows/release.yml)'s match guard. Instead, derive
the tag from `Cargo.toml`:

```powershell
./scripts/release.ps1            # tag v<Cargo.toml version> and push it
./scripts/release.ps1 -DryRun    # run every check + report; touch nothing
```

The script reads the version, refuses to run on a dirty tree / off `main` / for
an existing or non-advancing tag, then pushes an annotated `v<version>` tag.

As a belt-and-braces local backstop, enable the committed `pre-push` hook once
per clone so a mismatched tag is rejected even if you bypass the script:

```bash
git config core.hooksPath .githooks
```

[`.githooks/pre-push`](../.githooks/pre-push) is the local mirror of
`release.yml`'s tag-vs-`Cargo.toml` check — it blocks the push before a bad tag
ever reaches GitHub. (CI uses fresh checkouts with no `hooksPath`, so
`release.yml` remains the server-side backstop.)

Pushing the tag triggers [`release.yml`](../.github/workflows/release.yml) on a
Windows runner:

1. Installs LLVM (for `openvr_sys`'s bindgen step) and the WiX toolset.
2. Builds `cargo build --release -p ez-wishlist-overlay`.
3. Runs `cargo wix -p ez-wishlist-overlay --no-build` against [`crates/app/wix/main.wxs`](../crates/app/wix/main.wxs) to produce a per-machine MSI.
4. Attaches both the portable `ez-wishlist-overlay-<version>-portable.exe` and the `ez-wishlist-overlay-<version>-installer.msi` to a GitHub Release.

The `UpgradeCode` GUID in `main.wxs` is fixed — never change it once a release ships, or upgrades for existing installs break. The `Product/@Id='*'` regenerates per build so each MSI has a unique `ProductCode`.

`workflow_dispatch` is enabled, so you can fire a dry build from the Actions tab without tagging — artifacts are uploaded to the workflow run but skip the Release upload step.

---

## Conventions

- `cargo fmt` and `cargo clippy -- -D warnings` clean on every commit.
- Conventional-commit messages (`feat:`, `fix:`, `chore:`, …); `main` stays releasable, features land via PR.
- `unsafe` only in `vr/` for FFI, each with a `// SAFETY:` comment.
- No `unwrap()` / `expect()` in app code paths (tests are fine).
