...
- **`misc_b_pipeline` has display name "Valve" (id ≠ name) — keep the upstream slug, do NOT rename the id.** It was briefly renamed to `misc_b_valve` to match the label, but the scraper rebuilds the items catalog from upstream every run and silently reverted the catalog id back to `misc_b_pipeline` on the next scrape, leaving the hand-authored requirements pointing at a `misc_b_valve` id with no catalog entry (issue #89). Lesson: never rename a catalog id to match a display name — keep the upstream slug everywhere (requirements, fixtures, icon filename) and let `name` carry the in-game label. Other upstream-slug-vs-label mismatches (see the two notes above) follow the same rule.
