...
fn render_loop(
    ...
    // change, not every 90 Hz frame. The trailing `Option<Instant>` is the
    // active confirm's `shown_at` (None when none) — a fresh capture gets a
    // fresh stamp, so the marks re-render even if the chip text repeats, and drop
    // cleanly when the confirm expires. `None` = not shown.
                    ...
                    if let Some(s) = ocr_state.as_ref() {
                        if let Some(c) = s.feedback.chip_confirm() {
                            guide_confirm = Some(GuideConfirm {
                                label: c.0,
                                rgb: c.1,
                                // On-the-items marks only in the "OnItems" style;
                                // the other styles read off the centered card.
                                marks: if style.shows_on_item_marks() {
                                    guide_marks_from(&s.feedback)
                                } else {
                                    Vec::new()
                                },
...
/// reusing #138's per-item position plumbing: hideout `Done` → a count per cell
/// at `OcrItemDelta.pos`'s center; box/stash `BoxScanProgress` → a ✓/✗ per tile
/// of the current shot's `last_grid`. Other terminal kinds (NotAPanel / Failed /
/// …) have no per-item geometry, so no marks.
#[cfg(target_os = "windows")]
fn guide_marks_from(feedback: &crate::gui::OcrFeedback) -> Vec<super::guide::GuideMark> {
    ...
    match &feedback.kind {
        OcrFeedbackKind::Done { items, .. } => items
            .iter()
            .filter_map(|item| {
                item.pos.map(|p| {
                    let (x, y) = p.center();
            ...
            .collect(),
        OcrFeedbackKind::BoxScanProgress { last_grid, .. } => last_grid
            .iter()
            .flat_map(|row| {
                let y = row.y;
                row.cells.iter().map(move |cell| GuideMark::Check {
                    x: cell.x,
                    y,
