...
/// ([`CropMark`]); since the hole *is* the crop, the renderer maps them into the
/// hole sub-rect, so the marks line up with what the user is looking at.
#[derive(Clone, Copy, Debug, PartialEq)]
pub enum GuideMark {
    /// Hideout: the read owned-count at a panel cell. `None` = the cell was
    /// seen but its count couldn't be read (drawn as "?").
    Count { pos: CropMark, count: Option<u32> },
    /// Box / stash: a tile matched a catalog item (`true` → green ✓) or was
    /// detected but couldn't be read/matched (`false` → red ✗).
    Check { pos: CropMark, matched: bool },
}

impl GuideMark {
    fn pos(&self) -> CropMark {
        match self {
            GuideMark::Count { pos, .. } | GuideMark::Check { pos, .. } => *pos,
        }
    }
}
...
pub fn mark_px(pos: CropMark, layout: &GuideTexLayout) -> (f32, f32) {
    (
        layout.hole_x as f32 + pos.x.clamp(0.0, 1.0) * layout.hole_w as f32,
        layout.hole_y as f32 + pos.y.clamp(0.0, 1.0) * layout.hole_h as f32,
    )
}
...
fn draw_marks(pix: &mut Pixmap, marks: &[GuideMark], layout: &GuideTexLayout) {
    ...
    for mark in marks {
        let (cx, cy) = mark_px(mark.pos(), layout);
        match mark {
...
mod tests {
    ...
    fn mark_px_maps_normalized_into_the_hole() {
        ...
        assert_eq!(
            mark_px(CropMark { x: 0.0, y: 0.0 }, &l),
            (l.hole_x as f32, l.hole_y as f32)
        );
        assert_eq!(
            mark_px(CropMark { x: 1.0, y: 1.0 }, &l),
            ((l.hole_x + l.hole_w) as f32, (l.hole_y + l.hole_h) as f32)
        );
        assert_eq!(
            mark_px(CropMark { x: 0.5, y: 0.5 }, &l),
            (l.hole_x as f32 + 400.0, l.hole_y as f32 + 300.0)
        );
        // Out-of-range clamps to the hole edge (never into the margins).
        assert_eq!(
            mark_px(CropMark { x: -1.0, y: 2.0 }, &l),
    ...
    fn marks_paint_inside_the_hole() {
        ...
        let marks = [
            GuideMark::Count {
                pos: CropMark { x: 0.5, y: 0.5 },
                count: Some(7),
            },
            GuideMark::Check {
                pos: CropMark { x: 0.25, y: 0.5 },
                matched: true,
            },
            GuideMark::Check {
                pos: CropMark { x: 0.75, y: 0.5 },
                matched: false,
            },
        ];
        let pix = render(
        ...
        for (nx, what) in [(0.25f32, "✓"), (0.5, "count"), (0.75, "✗")] {
            let (mx, my) = mark_px(CropMark { x: nx, y: 0.5 }, &layout);
            assert!(
